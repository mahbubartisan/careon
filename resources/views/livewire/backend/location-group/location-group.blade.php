    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Location Group</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
    <div class="my-5 flex justify-end">
        @can("create location group")
            <a href="{{ route('create.location-group') }}"
            ...
            <table
                ...
                <thead class="bg-white dark:bg-[#132337]">
                    <tr>
                        ...
                        @if (auth()->user()->can("edit location group") || auth()->user()->can("delete location group"))
                            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">
                                Action
                            </th>
                        @endif
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($locationGroups as $index => $group)
                        <tr>
                            <td class="whitespace-nowrap px-6 py-4">{{ $locationGroups->firstItem() + $index }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $group->name }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $group->price }}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("edit location group")
                                        <a wire:navigate href="{{ route("edit.location-group", $group->id) }}"
                                            title="Edit">
                                            <!-- Edit Icon -->
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $locationGroups->firstItem() }}</span> to
                    <span>{{ $locationGroups->lastItem() }}</span>
                    of <span>{{ $locationGroups->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap -space-x-px rounded-md bg-white text-sm font-medium text-gray-700 shadow-sm dark:bg-gray-800 dark:text-gray-700"
                    ...
                    <!-- Pagination Numbers -->
                    @foreach ($locationGroups->links()->elements[0] as $page => $url)
                        @if ($page == $locationGroups->currentPage())
                            <span
                    ...
                    <!-- Next Button -->
                    @if ($locationGroups->hasMorePages())
