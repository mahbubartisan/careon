    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Care Level</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
                        ...
                        <tr>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("edit care level")
                                        <a href="{{ route("edit.care-level", $careLevel->id) }}" title="Edit">
                                            <!-- Edit Icon -->
                                    ...
                                    @endcan
                                    @can("delete care level")
                                        <button type="button" wire:click="delete({{ $careLevel->id }})"
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $careLevels->firstItem() }}</span> to
                    <span>{{ $careLevels->lastItem() }}</span>
                    of <span>{{ $careLevels->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap -space-x-px rounded-md bg-white text-sm font-medium text-gray-700 shadow-sm dark:bg-gray-800 dark:text-gray-700"
                    ...
                    <!-- Pagination Numbers -->
                    @foreach ($careLevels->links()->elements[0] as $page => $url)
                        @if ($page == $careLevels->currentPage())
                            <span
