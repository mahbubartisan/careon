    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">General Query</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($queries as $index => $query)
                        <tr>
                            <td class="whitespace-nowrap px-6 py-4">#{{ $queries->firstItem() + $index }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $query->name }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $query->phone }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $query->email }}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                {{ \Carbon\Carbon::parse($query->created_at)->format("F j, Y") }} •
                                {{ \Carbon\Carbon::parse($query->created_at)->format("g:i A") }}
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("view general query")
                                        <a href="{{ route("view.general-query", $query->id) }}" title="View">
                                            <!-- View Icon -->
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $queries->firstItem() }}</span> to
                    <span>{{ $queries->lastItem() }}</span>
                    of <span>{{ $queries->total() }}</span> entries
                </p>
            </div>
            ...
            <div>
                <nav class="relative z-0 inline-flex flex-wrap -space-x-px rounded-md bg-white text-sm font-medium text-gray-700 shadow-sm dark:bg-gray-800 dark:text-gray-700"
                    ...
                    <!-- Pagination Numbers -->
                    @foreach ($queries->links()->elements[0] as $page => $url)
                        @if ($page == $queries->currentPage())
                            <span
