    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Feedback</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($feedbacks as $index => $feedback)
                        <tr>
                            <td class="whitespace-nowrap px-6 py-4">#{{ $feedbacks->firstItem() + $index }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $feedback->name }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $feedback->phone }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $feedback->email }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $feedback->service }}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                {{ \Carbon\Carbon::parse($feedback->created_at)->format("F j, Y") }} •
                                {{ \Carbon\Carbon::parse($feedback->created_at)->format("g:i A") }}
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("view feedback")
                                        <a href="{{ route("view.feedback", $feedback->id) }}" title="View">
                                            <!-- View Icon -->
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $feedbacks->firstItem() }}</span> to
                    <span>{{ $feedbacks->lastItem() }}</span>
                    of <span>{{ $feedbacks->total() }}</span> entries
                </p>
            </div>
