    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Ambulance Bookings</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($bookings as $index => $booking)
                        <tr>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">{{ $booking->contact_person }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $booking->phone }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $booking->patient_name }}</td>
                            <td class="whitespace-nowrap px-6 py-4">
                                {{ \Carbon\Carbon::parse($booking->pickup_datetime)->format("F j, Y") }} •
                                {{ \Carbon\Carbon::parse($booking->pickup_datetime)->format("g:i A") }}
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    @can("booking detail")
                                        <a href="{{ route("ambulance.booking.detail", $booking->id) }}" title="View">
                                            <!-- View Icon -->
