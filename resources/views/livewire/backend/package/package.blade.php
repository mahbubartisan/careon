    ...
    <div class="my-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="text-[13.5px] text-gray-700 dark:text-gray-300">Package</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <table
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-[#233A57] dark:bg-[#132337]">
                    @forelse ($packages as $index => $package)
                        <tr>
                            <td class="whitespace-nowrap px-6 py-4">{{ $packages->firstItem() + $index }}</td>
                            <td class="whitespace-nowrap px-6 py-4">{{ $package->name }}</td>
        ...
        <div class="mt-4 flex flex-col items-center justify-between gap-y-3 sm:flex-row">
            ...
            <div>
                <p class="text-sm text-gray-700 dark:text-gray-700">
                    Showing <span>{{ $packages->firstItem() }}</span> to
                    <span>{{ $packages->lastItem() }}</span>
                    of <span>{{ $packages->total() }}</span> entries
                </p>
            </div>
